...
class DataLoader:
    ...
    def read_dcm(self, img_path, is_gt):
    ...
    def _get_batch(self, paths_batch, stage):
        ...
        for path in paths_batch:
            ...
            target_path = os.path.join(path, '{}_{}.dcm'.format(patient_id, self.target_name))
            condition_path = os.path.join(path, '{}_{}.dcm'.format(patient_id, self.condition_name))
            input_path = os.path.join(path, '{}_{}.dcm'.format(patient_id, self.input_name))

            input_img = self.read_dcm(input_path, '_gt' in self.input_name)
            if self.augment['AUG_INPUT']:
                input_img = self.datagen.apply_transform(input_img, transform)
            input_imgs.append(input_img)

            target_img = self.read_dcm(target_path, '_gt' in self.target_name)
            condition_img = self.read_dcm(condition_path, 1)
